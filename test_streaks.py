...
def test_streaks():
    assert compute_streaks(set(), OLD, TODAY) == (0, 0, 0, set())
    assert compute_streaks({d(0)}, OLD, TODAY) == (1, 1, 0, set())
    # today not ticked yet, but yesterday+ chain is still live
    assert compute_streaks({d(1), d(2)}, OLD, TODAY) == (2, 2, 0, set())
    # missed yesterday and today -> current dead, longest remembered
    assert compute_streaks({d(2), d(3)}, OLD, TODAY) == (0, 2, 0, set())
    ...
    assert compute_streaks({d(i) for i in range(6, 13)} | {d(i) for i in range(1, 5)}, OLD, TODAY) == (12, 12, 0, {d(5)})

    # logs older than the habit's creation date don't count
    assert compute_streaks({d(0), d(1), d(2)}, d(1), TODAY) == (2, 2, 0, set())
